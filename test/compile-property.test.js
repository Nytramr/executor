...
describe('Compile Property', () => {
  ...
  describe('using accessors', () => {
    ...
    it('should return the value of the key obtained by a complex property', () => {
      ...
      const executor2 = engine.compile('PP([key.sub-key])');
      expect(executor2({ value: 'name', key: { 'sub-key': 'value' } })).toBe('name');
      expect(executor2({ 'another value': 'another name', key: { 'sub-key': 'another value' } })).toBe('another name');
      ...
      const executor3 = engine.compile('PP(PP(key.sub-key))');
      expect(executor3({ value: 'name', key: { 'sub-key': 'value' } })).toBe('name');
      expect(executor3({ 'another value': 'another name', key: { 'sub-key': 'another value' } })).toBe('another name');
      expect(executor3({ value: 'name', keyNotFound: { 'sub-key': 'value' } })).toBeUndefined();
      expect(executor3({ value: 'name', key: { 'sub-keyNotFound': 'value' } })).toBeUndefined();
      expect(executor3({})).toBeUndefined();
    });

    it('should return the value of the key obtained by a simple property once in a dipper property context', () => {
      const executor = engine.compile('context[key]');
      ...
      const executor2 = engine.compile('PP(context[key])');
      expect(executor2({ context: { value: 'name' }, key: 'value' })).toBe('name');
      expect(executor2({ context: { 'another.value': 'another name' }, key: 'another.value' })).toBe('another name');
      expect(executor2({ context: { value: 'name' }, keyNotFound: 'value' })).toBeUndefined();
      expect(executor2({})).toBeUndefined();
    });
  });
